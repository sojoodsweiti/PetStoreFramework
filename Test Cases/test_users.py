...
def test_create_user(base_url, user_payload):
    response = requests.post(f"{base_url}/user", json=user_payload)
    assert_that(response.status_code).is_equal_to(200)


def test_get_user_by_username(base_url, user_payload):
...
def test_update_user(base_url, user_payload):
    username = user_payload["username"]
...
def test_delete_user(base_url, user_payload):
    username = user_payload["username"]
